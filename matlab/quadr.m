...
if qtype=='g' % global (ptr) quadr
    ...
    elseif isfield(s,'x')
        s.x = s.x(:);
        N = numel(s.x);
        % fill in missing part, assumed t as below
        t = (1:N)'/N*2*pi;
        s.p = N; s.w = 2*pi/N*ones(N,1); np=1;
        xhat = fft(s.x)/N;
        if mod(N,2) == 0
          k = [0:N/2-1, 0, -N/2+1:-1]';
        else
        ...
        s.Z = @(t) reshape(xhat.' * exp(1i * k * t(:).'), size(t));
    else
...
if isfield(s,'Zp'), s.xp = s.Zp(t); % 1st dir of curve
    ...
    if qtype == 'p'
        ...
        %
        xphat = xhat .* (1i * k);
        s.Zp  = @(t) reshape(xphat.'  * exp(1i * k * t(:).'), size(t));
    end
end
if isfield(s,'Zpp'), s.xpp = s.Zpp(t); % 2nd dir of curve
    ...
    for i=1:np
        ...
        %
        xpphat = xhat .* (-(k.^2)); 
        s.Zpp = @(t) reshape(xpphat.' * exp(1i * k * t(:).'), size(t));
    end
end
